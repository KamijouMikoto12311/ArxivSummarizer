...
def _summarize_pdf_text(pdf_text, api_key, model="Pro/deepseek-ai/DeepSeek-V3"):
    ...
    prompt = (
        ...
        "摘要应易于理解，并且长度应合理，不应过长或过短，约在600字左右，严格控制不得超过800字！"
        "在开头应指出文章的几个关键词."
    )
...
def _process_single_pdf(
    ...
    pdf_text = extract_section(pdf_path)
...
def process_pdfs_in_directory(pdf_dir: str, processed_dir: str, api_key: str, output_dir: str, output_name: str = "summary.txt"):
    file_lock = threading.Lock()
...
def main():
    ...
    process_pdfs_in_directory(pdf_directory, processed_dir, api_key, output_directory)
